    ...
    return Scaffold(
      ...
      body: Obx(
          ...
          itemBuilder: (context, index) {
            return ListTile(
              leading: CachedNetworkImage(imageUrl: dbController.dBList.value[index].image!,height: 50,width: 50,fit: BoxFit.cover,placeholder: (context, url) => const Icon(Icons.shopping_cart),errorWidget: (context, url, error) => const Icon(Icons.shopping_cart),),
              title: Text(dbController.dBList.value[index].name!),
              subtitle: Text(dbController.dBList.value[index].dec!),
              trailing: Row(
                ...
                children: [
                  ...
                  IconButton(onPressed: () {
                    Get.defaultDialog(title: "Are You Sure ?",content: Text("Are You Sure To Delete ${dbController.dBList[index].name} ?")
                        ,actions: [
                          ...
                          ElevatedButton(onPressed: () async {
                            await DBHelper.helper.delete(dbController.dBList[index].id!);
   ...
   Get.defaultDialog(title: "Add Products",content: Form(
     ...
     child: Column(
       ...
       children: [
         ...
         Obx(() =>
             Slider(value: dbController.sliderP.value, onChanged: (value) {
               dbController.sliderP.value=value;
             },max: 1000,min: 0,divisions: 10,),
         ),
         const SizedBox(height: 2,),
         Obx(() => Center(child: Text(dbController.sliderP.value.toString())),)
       ],
     ),
   ),
     actions: [
       ...
       Obx(
           ...
           if(key.currentState!.validate()) {
             if (model1 == null) {
               DBModel model = DBModel(price: dbController
                   .sliderP.value.toString(),
                   name: nameTxt.text,
                   dec: decTxt.text,
                   image: imageTxt.text);
